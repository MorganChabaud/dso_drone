...
namespace IOWrap
{
MinimalImageB* readImageOnline(cv::Mat & img)
{
	if(img.rows*img.cols==0)
	{
		printf("Unable to capture image! this may segfault. \n");
		return 0;
	}
	if(img.type() != CV_8U)
	{
		printf("The capture did not output a gray image (CV_8U). \n");
		return 0;
	}
	MinimalImageB* minImg = new MinimalImageB(img.cols, img.rows);
	memcpy(minImg->data, img.data, img.rows*img.cols);
	return minImg;
}
